...
- **Parcours acheteur (PWA)** : à la confirmation d'une commande client, WAZAP envoie au client le lien `suivi.html?id=…` ; le client valide **position/adresse** → déclenchement **automatique** de la recherche des livreurs (`/api/client/orders/{id}/coordinates`, migration `AddBuyerTracking`).
